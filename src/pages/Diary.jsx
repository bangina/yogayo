...
const Diary = () => {
  ...
  const lessonCall = () => {
    ...
    const apiUrl = `http://api.yogayo.kr/api/diaries/lessons/`;
    axios
  ...
  const apiCall = (pageNum) => {
    let cookies = new Cookies();
    const userToken = cookies.get("usertoken");
    const apiUrl = `http://api.yogayo.kr/api/diaries/mydiaries/?page=${pageNum}`;
    axios
      .get(apiUrl, { headers: { Authorization: `Token ${userToken}` } })
      .then((response) => {
  ...
  return (
    <div>
      ...
      <DiaryModal />
      <ResultModal
        directTo="/diary/mydiary"
        message="다이어리가 정상적으로 게시되었습니다."
        header="수련일기"
      />
    </div>
  );
